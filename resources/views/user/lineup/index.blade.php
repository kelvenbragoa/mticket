        ...
        @endif
            <div class="member-place-wrap">
                <div class="member-wrap-top">
                    <h2>LineUp do Evento - {{$event->name}} </h2>  
                </div>
                <!-- .member-wrap-top -->
                ...
                <table class="member-place-list table-responsive">
                    ...
                    <tbody>
                        @forelse ($lineup as $item)
                            <tr>
                            
                                <td data-title="Nome">{{$item->name}}</td>
                                <td data-title="Preço">{{$item->description}}</td>
                                ...
                                <td data-title="" class="place-action">
                                    <a href="{{URL::to('/lineup/'.$event->id.'/evento/'.$item->id.'/edit')}}" class="edit" title="Edit"><i class="las la-edit"></i></a>
                                    <form method="POST" action="{{ route('lineup.destroy', $item->id)}}">
                                        @csrf
                                        @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="7">Nenhum Lineup para este evento !</td>
