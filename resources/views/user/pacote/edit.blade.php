...
<main id="main" class="site-main listing-main">
    ...
    <div class="listing-content">
        ...
        <h2>Editar Pacote - {{$package->name}} - {{$event->name}}</h2>
        <a href="{{URL::to('/pacote/'.$event->id.'/evento')}}" title="Genaral"><span class="icon"><i class="la la-angle-left"></i></span><span>Voltar</span></a>
        <form class="upload-form" method="POST" action="{{ route('packages.update', $package->id)}}" enctype="multipart/form-data">
            @csrf
            @method('PATCH')
            <div class="listing-box" id="genaral">
                ...
                <div class="field-group field-select">
                    ...
                    <input type="text" id="name" placeholder="Nome do evento" name="name" value="{{$package->name}}">
                   
                </div>
                <div class="field-group field-select">
                    <label for="lis_category">Preço do Pacote</label>
                    <input type="number" id="price" placeholder="Preço do Pacote" name="price" value="{{$package->price}}">
                   
                </div>
                
                <div class="field-group">
                    <h4 class="label">Descrição do Pacote</h4>
                    <textarea type="text" id="description" class="form-control" placeholder="Descrição do Pacote" name="description" rows="5">{{$package->description}}</textarea>
